...
def parse_uptime(uptime: str) -> timedelta:
    """Парсинг строки uptime в timedelta"""
    seconds = timeparse(uptime)
    if seconds is not None:
        return timedelta(seconds=seconds)
    else:
        log.error(f"Ошибка парсинга: '{uptime}'")
        return timedelta(0)


def parse_percent(val: str) -> int:
    """Парсинг строки с процентами в целое число"""
    try:
...
def poll_all_servers():
    """Задача для опроса всех серверов и сохранения их статусов"""
    ...
    for server in RemoteServer.objects.all():
        try:
            ...
            log.error(
                f"Ошибка при опросе сервера {server.name} ({server.url}): {e}"
            )
        else:
